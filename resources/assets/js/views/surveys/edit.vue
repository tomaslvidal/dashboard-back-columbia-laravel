<template>
	<div v-if="Object.keys(item).length>0">
		...
		<b-form @submit="onSubmit" v-if="show">
			...
			<b-button type="submit" variant="primary" :disabled="disabledCreate">Editar</b-button>
		</b-form>
	</div>
</template>
...
export default {
	...
	created(){
		this.$store.dispatch('Surveys/GET_ITEM', { "id" : this.$route.params.id});
	...
	destroyed(){
		this.$store.dispatch('Surveys/CLEAR_ITEM');
	},
	computed: {
		item(){
			if(!this.state.checkNumberQuestions){
				if(this.$store.state.Surveys.item.survey_fields){
					this.model.numberQuestions = this.$store.state.Surveys.item.survey_fields.length;

					this.show = true; this.state.checkNumberQuestions = true;
				}
			}
		...
		onSubmit(evt){
			...
			.then(()=>{
				setTimeout( () => {
					...
					this.$store.dispatch('Surveys/UPDATE_ITEM', item);
